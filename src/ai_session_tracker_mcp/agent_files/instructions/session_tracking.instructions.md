...
Flow: start_ai_session(name,type,model,mins,source,developer,project) → session_id → log_ai_interaction(session_id,rating,notes) (≥1) → end_ai_session(session_id,outcome,final_estimate_minutes)
